...
FILTER_ALPHA   = 0.2       # smoothing factor for IMU readings
HISTORY_WINDOW = 100       # how many past entries to keep
...
def animate_checks(stop_event):
    """Show running checks with dot animation on bottom row."""
    idx = 0
    start_col = (16 - 3) // 2  # leftmost column for dot animation

    if lcd1:
        lcd1.clear()
        lcd1.write_string("Running checks".center(16))
    if lcd2:
        lcd2.clear()
        lcd2.write_string("Running checks".center(16))
    ...
    while not stop_event.is_set():
        ...
        if lcd1:
            lcd1.cursor_pos = (1, 0)
            lcd1.write_string(line)
        if lcd2:
            lcd2.cursor_pos = (1, 0)
            lcd2.write_string(line)
        idx += 1
        time.sleep(0.5)
...
def startup_checks():
    ...
    # Show final status on both LCDs if available
    final_msg = "All systems OK" if not errors else f"{len(errors)} errors found"

    if lcd1:
        lcd1.clear()
        if not errors:
            lcd1.write_string(final_msg.center(16))
        else:
            lcd1.write_string(final_msg.ljust(16)[:16])
    if lcd2:
        lcd2.clear()
        if not errors:
            lcd2.write_string(final_msg.center(16))
        else:
            lcd2.write_string(final_msg.center(16)[:16])
...
try:
    while True:
        ...
        # B) READ IMU tilt with simple smoothing to filter noise
        if sensor:
            euler = sensor.euler
        else:
            euler = None
        ...
        if euler is None:
            roll_raw, pitch_raw = None, None
        else:
            _, roll_raw, pitch_raw = euler

        if roll_raw is not None and pitch_raw is not None:
            if filt_roll is None:
                filt_roll = roll_raw
                filt_pitch = pitch_raw
            else:
                filt_roll += FILTER_ALPHA * (roll_raw - filt_roll)
                filt_pitch += FILTER_ALPHA * (pitch_raw - filt_pitch)
            roll, pitch = filt_roll, filt_pitch
        else:
